...
From the command line you can select the backend using the :option:`--backend` option.
...
Option :option:`--db-file` sets the name of the file to save the "DB",
default is ``.doit.db``.
...
of report. This is useful when used in conjunction with :option:`--continue` option.
...
You can specify a different *cwd* with the :option:`-d`/:option:`--dir` option.
...
By default the execution of tasks is halted on the first task failure or error. You can force it to continue execution with the option :option:`--continue`/:option:`-c`
...
The option :option:`-s`/:option:`--single` can be used to execute a task without executing
...
`doit` supports parallel execution :option:`--process`/:option:`-n`.
...
(see :ref:`file-dep`).  Use the option :option:`--check_file_uptodate` to choose:
...
The option :option:`--output-file`/:option:`-o` let you output the result to a file.
...
If the option :option:`--pdb` is used, a post-mortem debugger will be launched in case
...
as the current working directory (unless :option:`--dir` is specified).
...
By specifying :option:`--auto-delayed-regex`, every delayed task loader having no
