...
different backends:
 ...
 - `json`: Plain text using a :abbr:`JSON (JavaScript Object Notation)` structure, it is slow but good for debugging.

 - `sqlite3`: Support concurrent access
   (:abbr:`DB` is updated only once when process is terminated for better performance).
 ...
 * json: Output results in :abbr:`JSON` format
...
`doit` uses the ``json`` package to serialize and deserialize values returned
by python-actions. If required an alternate encoder/decoder pair may be
specified so that return values which are not :abbr:`JSON` serializable, such
as Python class instances, may be provided as return values from
python-actions.
